...
public class MainPageObject {
    ...
    public void clickElementByPosition(String locator, int position, String error_message) {
        WebElement element = this.waitForElementPresent(locator + "/..", error_message);
        int width = element.getSize().getWidth();
        ...
        int upper_y = element.getLocation().getY();
        int lower_y = upper_y + height;

        int point_to_click_x = (left_x + right_x) / 2;
        int point_to_click_y = (upper_y + lower_y) / 2 + (height * (position - 1));
    ...
    public List getElements(String locator) {
        By by = this.getLocatorByString(locator);
        return driver.findElements(by);
    }

    public String getTextFromElementByPosition(String locator, int position) {
        List<WebElement> elements = getElements(locator);
        if (elements.size() < position) {
            throw new AssertionError("Element by position" + position + "does not exist");
        }
        if (Platform.getInstance().isAndroid()) {
            return elements.get(position - 1).getAttribute("text");
        } else {
            return elements.get(position - 1).getAttribute("name");
        }
    }
